...
What you should see are three columns of numbers, each row representing a 
different year: the first is the year, and the second and third are the numbers of boys and girls baptized that year, respectively. Tap the 'next' on the right side of the console 
window to examine the complete data set. R has stored Arbuthnot's data in a kind of spreadsheet or table called a *data frame*.

You can see the dimensions of this data frame by typing:

```{r dim-data, eval=TRUE}
dim(arbuthnot)
myrowcount <-nrow(arbuthnot)
class(myrowcount)
ncol(arbuthnot)
class(arbuthnot)
```
...
the object in your workspace. The nrow() and ncol() give you the rows and columns as a number with class"integer".  This can be very helpful when you need to identify proportions of data. You can see the names of these columns (or variables) by typing:
...
-   In what year did we see the highest total number of births in the U.S.? You can
